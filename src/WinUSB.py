...
class IsoCopy:
    ...
    def __init__(self, iso_path, drive):
        ...
        self.isoTmpFolder = "/run/mauna-iso-tmp/"
        self.usbMountFolder = "/run/mauna-usb-tmp/"
    ...
    def mountFolders(self):
        ...
        if not os.path.exists(os.path.dirname(os.path.abspath(__file__)) + "/../uefi-ntfs.img"):
            run(["wget", "-c", "https://github.com/pbatard/rufus/raw/master/res/uefi/uefi-ntfs.img", "-O",
                 os.path.dirname(os.path.abspath(__file__)) + "/../uefi-ntfs.img"])
        run(["dd", "if=" + os.path.dirname(os.path.abspath(__file__)) + "/../uefi-ntfs.img",
             "of={}2".format(self.drive)])
        run(["sync"])
