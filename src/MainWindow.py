...
class MainWindow:
    def __init__(self, application, file=""):
        ...
        self.dialog_about.set_program_name(_("Mauna Image Writer"))
    ...
    def onCheckingIntegrityFinished(self):
        ...
        if isISOGood:
            ...
            dialog.format_secondary_text(
                _("This is not a Mauna ISO, or it is corrupted.")
            )
            dialog.run()
            dialog.destroy()
    ...
    def prepareWriting(self):
        ...
        # Get MD5SUMS from maunalinux.top:
        try:
            result = requests.get("http://cdimage.maunalinux.top/mauna/MD5SUMS")
            ...
            dialog.format_secondary_text(
                _("Could not connect to maunalinux.top.")
            )
            dialog.run()
            dialog.destroy()
    ...
    def sendNotification(self, title, body):
        ...
        notification.set_icon(Gio.ThemedIcon(name="mauna-image-writer"))
